...
At this time there is a known issue that happenes if the checkout process
is abandoned and then returned to. The tax amount does not get updated in
the cart.

Note:  spree_tax_cloud does not handle multiple tax category tic's
